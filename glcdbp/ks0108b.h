...
void     ks0108bDisplayOff(void);  // added display Off
...
void	 ks0108bFillArea(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t c);
...
NOTE the autoincrement of Y only works within a "display" once y hits 64 you
want to move to the 2nd "display" but Y wraps at 63.  the 2nd display has it's own
Y which is where it was last left so when you move from display 1 to display 2
you have to reset display 2 Y 
