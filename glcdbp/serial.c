...
void serialInit(uint16_t baudRate)
{
  ...
  UCSR0A = (1<<U2X0);			// u2xo double the transmission speed
  //UCSR0B = (1<<RXCIE0)|(1<<RXEN0)|(1<<TXEN0);  //Enable Interrupts on receive
  UCSR0B = (1<<RXCIE0)|(1<<RXEN0);  //Enable Interrupts on receive, DON'T ENABLE THE TRANSMITTER SO THAT WE CAN USE IT FOR BUFFER CONTROL
...
char serialBufferPop(void)
{
  cli();
  bufferSize--;
  if(bufferSize < 16) PORTD &= ~(0x02);	// if we just emptied the buffer enough, tell sender it's OK to transmit again
...
void clearBuffer(void)
{
  ...
  rxRingTail = 0;
  rxRingHead = 0;
  PORTD &= ~(0x02);	// if we'd turned flow off, turn it back on
  sei();
}
