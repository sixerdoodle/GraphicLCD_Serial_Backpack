...
extern uint8_t column;  // should be elsewhere, used in ks0108b.h
...
void lcdDrawChar(char printMe)
{
  ...
  switch(printMe)
  {
    case '\r':  // Newline.
		...
		if (cursorPos[Y] >= (yDim-7)) cursorPos[Y] = textOrigin[Y];
		break;
    ...
    case '\b':
		if (textLength > 0) // no text, no backspace!
		{
		  ...
		  if (cursorPos[X] == textOrigin[X])
		  {
			...
			if (cursorPos[Y] == textOrigin[Y])
			{
				while (cursorPos[Y] < (yDim-8)) cursorPos[Y] +=8;
				while (cursorPos[X] <= (xDim-6)) cursorPos[X] += 6;
				cursorPos[X]-=6;
			}
		  ...
		  for (uint8_t x = cursorPos[X]; x < cursorPos[X]+5; x++)
		  {
			for (uint8_t y = cursorPos[Y]; y < cursorPos[Y]+8; y++)
			{
  ...
  //  that we can print out. That's everything between space and tilde.
	if ((printMe >= ' ') && (printMe <= '~'))
	{
		...
		if ( (display == SMALL) && ( (cursorPos[Y] & 0x07) == 0) ){	// then we're on a full byte row, so we can skip the draw pixel					
			ks0108bFastText(cursorPos[X],cursorPos[Y],charOffset);
		} else {
			for (uint8_t x = cursorPos[X]; x < cursorPos[X]+5; x++)
			{
				...
				for (uint8_t y = cursorPos[Y]; y < cursorPos[Y]+8; y++)
				{
					if ( (colTemp >> (y-cursorPos[Y]) ) & 0x01) lcdDrawPixel(x,y,ON);
					else lcdDrawPixel(x,y,OFF);
				}
			}	
		}
		cursorPos[X] += 6;  // Increment our x position by one character space.
		// if we're at the end of the line, we need to wrap to the next line.
		if (cursorPos[X] >= (xDim-6))
		{
		  cursorPos[X] = textOrigin[X];
		  cursorPos[Y] += 8;
		  if (cursorPos[Y] >= (yDim-7)) cursorPos[Y] = textOrigin[Y];
...
void lcdEraseBlock(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
  ...
  //   down y.
  if(display == SMALL){	
	ks0108bFillArea(x0, y0,x1,y1,0x00);  // quick fill for the ks'  allows fill row by bytes for full rows
  } else {
...
void lcdDrawLogo(void)
{
  ...
  // spit out the compile date/time of the firmware we're running
  char s[32];
