...
extern volatile uint8_t		bufferSize;   // has to be 16 bit too if BUF_DEPTH=256
...
void uiStateMachine(char command)
{
  ...
  switch(command)
  {
    ...
    // trigger command that simply toggles one of the debug I/O lines so we can
    // start capturing data at a specific spot if we want
    case TRIGGER:  
      PORTD |= 0x02;	
	  _delay_us(10);
	  PORTD &= ~(0x02);
    ...
    case ADJ_TEXT_X: // This is the x-origin of our text "window".
        ...
        {
        textOrigin[X] = cmdBuffer[0];
        cursorPos[X] = textOrigin[X];
        textLength = 0;
        }
    ...
    case ADJ_TEXT_Y:
		...
		{
			textOrigin[Y] = cmdBuffer[0];
			cursorPos[Y] = textOrigin[Y];
			textLength = 0;
		}
    ...
    // command to test the serial transmission.  whatever the next byte is
    // we expect to continue to see that and only that character.  if we see
    // a different characters display an X as that means we had a transmission error
    // THIS COMMAND NEVER EXITS you must power cycle to come out.  intent is you
    // can run this for hours and count the number of X's indicating the number of
    // transmission errors.
	case SERIAL_TEST:
		while (bufferSize == 0);    // ...wait for the next character...
		cmdBuffer[0] = serialBufferPop();  // this is the byte we need to repeat forever
		...
		{
			while(bufferSize == 0);					// ...wait for the next character...
			if(cmdBuffer[0] == serialBufferPop()){
				// good char, the transmit was a success
			} else {
				// bad char, the transmit ended up looking like a different character
				lcdDrawChar('X');
			}
		}
