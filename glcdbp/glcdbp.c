...
int main(void)
{
  ...
  while(1)
  {
	...
	while (bufferSize == 0);    // ...wait for the next character..., reformed loop so it's more sequential to read

	// serialBufferPop() pulls data from the top of the FIFO that comprises
	//  our serial port buffer, automatically changing the pointers and stack size.
	char bufferChar = serialBufferPop();
	
	if (bufferChar == '|')			// If the character received is the command escape character ('|')...
	{
		while (bufferSize == 0);    // ...wait for the next character (all escape are at least escape + 1 byte)
	...
	else if (((bufferChar >= ' ') && (bufferChar <= '~')) ||  // printable
